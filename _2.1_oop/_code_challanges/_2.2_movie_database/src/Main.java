public class Main {
    public static void main(String[] args) {
        ...
        studioMoreThan2Movies(studioDatabase);
        System.out.println("Cerinta 2");
        studioWithName(studioDatabase);
        System.out.println("Cerinta 3");
        movieActorAbove50(studioDatabase);

    }

    public static void studioMoreThan2Movies(Studio[] storage) {
        for (Studio studio : storage)
            if(studio.getSize() > 2)
                System.out.println(studio.getNume());
    }

    public static void studioWithName(Studio[] stoarge) {
        for (Studio studio : stoarge) {
            boolean exists = false;
            for (Film film : studio.getFilm())
    ...
    public static void movieActorAbove50(Studio[] storage) {
        for (Studio studio : storage)
            for (Film film : studio.getFilm()) {
                boolean exists = false;
                for (Actor actor : film.getActor())
